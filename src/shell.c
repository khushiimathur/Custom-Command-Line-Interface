...
void textToHex(const char *inputFile, const char *outputFile) {
    ...
    if (input_file == NULL || output==NULL) {
        printf("Error: Could not open file(s).\n");
        return;
    }
...
void textToBase64(const char *inputFile, const char *outputFile) {
    ...
    while ((bytesRead = fread(buffer, 1, 3, input_file)) > 0) {
...
void binaryToText(const char *inputFile, const char *outputFile) {
    FILE *input_file = fopen(inputFile, "rb");
    FILE *output = fopen(outputFile, "w");
    if (input_file == NULL || output==NULL) { //!input_file || !output
        printf("Error: Could not open file(s).\n");
        return;
    }
...
int main(int argc, char** argv) {
    ...
    while (run) {
        ...
        // convert
            ...
            const char *inputFile = input[1];
            const char *outputFile = input[2];
            const char *mode = input[3];
